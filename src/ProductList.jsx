...
function ProductList({ onHomeClick }) {
  ...
  const totalItemsInCart = useSelector((state) =>
    state.cart.items.map((item) => item.quantity).reduce((a, b) => a + b, 0),
  );
    ...
    <div>
      ...
      {!showCart ? (
        <div className="product-grid">
          {plantsArray.map((group, id) => {
            return (
              <div key={id}>
                <h2 style={styleH2}>{group.category}</h2>
                <div className="product-list">
                  {group.plants.map((plant, plantId) => {
                    return (
                      <div key={plantId} className="product-card">
                        ...
                        <button
                          ...
                          disabled={cart.includes(plant.name)}
                          onClick={() => handleAddToCart(plant)}
                        >
                          {cart.includes(plant.name)
                            ? "Added to Cart"
                            : "Add to Cart"}
                        </button>
                      </div>
